...
const prizeTable = [
  { label: "❌ Rien",              icon: "😔", isWin: false },
  { label: "🏷️ -10% Solde",       icon: "🎉", isWin: true  },
  { label: "🚚 Livraison gratuite", icon: "🎉", isWin: true  },
  { label: "🎁 Cadeau mystère",    icon: "🎁", isWin: true  },
  { label: "❌ Rien",              icon: "😔", isWin: false },
  { label: "🏷️ -20% Solde",       icon: "🎉", isWin: true  },
  { label: "☕ Café offert",       icon: "☕", isWin: true  },
  { label: "🏷️ -5% Solde",        icon: "🎉", isWin: true  },
  { label: "❌ Rien",              icon: "😔", isWin: false },
  { label: "🏆 Grand Prix !",      icon: "🏆", isWin: true  },
];
    ...
    if (window.start3) {
      ...
      if (inc > 0) {
        ...
        if (mouse.isSpinning == true) {
          ...
          let prize = prizeTable[answer - 1];
          // Utilisation de l'overlay HTML au lieu d'une alerte
          const overlay = document.getElementById("gift-overlay");
          const badge = document.getElementById("gift-badge");
          if (overlay && badge) {
            badge.innerText = `${prize.label} ${prize.icon}`;
...
window.resetSpin = () => {
  inc = 0;
  mouse.isSpinning = false;
  // On referme le panneau de résultat s'il est ouvert
  if (revealAnim.isShowing && !revealAnim.isAnimating) {
    animAction.paused = false;
    revealAnim.isAnimating = true;
    animAction.setLoop(THREE.LoopOnce);
    animAction.timeScale = -1;
